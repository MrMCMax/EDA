...
public class ABB<E extends Comparable<E>> {
    ...
    public String datosEnNivel(int k) {
        ...
        return "[" + datosEnNivelV2(k, this.raiz) + "]";
    }
    
    public String datosEnNivel(int k, NodoABB<E> actual, int nivelActual) {
        String res = "";
        if (actual != null) {
            if (k == nivelActual) {
                res = actual.dato.toString();
            } else {
                res = datosEnNivel(k, actual.izq, nivelActual + 1) +
                ", " + datosEnNivel(k, actual.der, nivelActual + 1);
            }
        }
        return res;
    }   //Se puede quitar el argumento "nivelActual"??
    
    public String datosEnNivelV2(int k, NodoABB<E> actual) {
        String res = "";
        if (actual != null) {
            if (k == 0) {
                res = actual.dato.toString();
            } else {
                res = datosEnNivelV2(k - 1, actual.izq) +
                ", " + datosEnNivelV2(k - 1, actual.der);
            }
        }
        return res;
    }
    
    public int alturaDeEquilibrado(NodoABB<E> actual) throws NoSuchElementException{
        if (actual == null) {
            return -1; 
        } else {
