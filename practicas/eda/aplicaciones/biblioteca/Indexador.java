...
public class Indexador {
            ...
            while (libro.hasNext()) {
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    if (esTermino(clave)) {
                        ...
                        if (lista == null) {
                            lista = new LEGListaConPI<Indice>();
                        } 
                        lista.insertar(ind);
                        indices.insertar(clave, lista); //Insertar o actualizar
                        t2 = System.nanoTime();
                    }
                    tmp += (t2 - t1);
                }
    ...
    public ListaConPI<String> indiceDe(String pal) {
        ...
        // COMPLETAR
        ListaConPI<Indice> lista = indices.recuperar(pal.toLowerCase());
        if (lista != null) {
            lista.inicio();
            while (!lista.esFin()) {
                res.insertar(lista.recuperar().toString());
                lista.siguiente();
            }
        }
        return res;   
